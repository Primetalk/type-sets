...
sealed trait TSetProperties extends TSetBase {
  type BelongsTo[Element, S <: TSet] <: Boolean = S match
    ...
    case SymmetricDifference[a, b] =>
      BelongsTo[Element, Union[a, b]] && ![BelongsTo[Element, Intersection[a, b]]]
  ...
  type FoldLeft[S <: TSet, Res, Z <: Res, Combine[_ <: Res, _] <: Res] <: Res = S match
    ...
    case Union[a, b] =>
      FoldLeft[b, Res, FoldLeft[a, Res, Z, Combine], [r,
      el] =>> If[BelongsTo[el, a], r, Combine[r, el]]]
    ...
    case SymmetricDifference[a, b] =>
      FoldLeft[b, Res, FoldLeft[a, Res, Z, [r, el] =>> If[BelongsTo[el, b], r, Combine[r, el]]], [r,
      el] =>> If[BelongsTo[el, a], r, Combine[r, el]]]
  type FoldRight[S <: TSet, Res, Z <: Res, Combine[_, _ <: Res] <: Res] <: Res = S match
    ...
    case Union[a, b] =>
      FoldRight[a, Res, FoldRight[b, Res, Z, Combine], [el,
      r] =>> If[BelongsTo[el, b], r, Combine[el, r]]]
    case Intersection[a, b] =>
